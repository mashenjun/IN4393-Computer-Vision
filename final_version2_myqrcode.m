%% start
load('Tables.mat');
QR = imread('input/Hello_phone17.jpg');
gray_QR =rgb2gray(QR);
...
temp_QR_binary_invert = bwareaopen(temp_QR_binary_invert, 40);
...
[~,dDL] = min(DIAG2);    [m,dUR] = max(DIAG2);
    ...
    if R(i).Area >3 
        ...
        ALL_Point_outer_format2(:,2*i-1:2*i) = [-1,-1;-1,-1;-1,-1;-1,-1];
    end
end
...
for i=1:size(mat_R_Area_index,2)
    ...
    if sum(in)-sum(on) == 4      
        ...
        if round(area_ratio) >=4 && fix(area_ratio)<=6
            find_num = find_num+1;
...
for i=1:NR
    if  R(i).Area >max_outer_Area || R(i).Area < min_outer_Area
        if pdist([[mean([max(R(i).PixelList(:,1)),min(R(i).PixelList(:,1))]),mean([max(R(i).PixelList(:,2)),min(R(i).PixelList(:,2))])];Center_outer_triangle])> 0.9*mean(triangle_length)
            for j = 1:size(R(i).PixelList,1)
                temp_QR_binary_invert(R(i).PixelList(j,2),R(i).PixelList(j,1))=0;
            end
        end
    end
end
...
temp_QR_binary_invert_H3(Harris_area(3,3):Harris_area(3,4),Harris_area(3,1):Harris_area(3,2)) = 1;
QR_Corner_1 = corner(temp_QR_binary_invert.*temp_QR_binary_invert_H1,'Harris','QualityLevel',0.2);
QR_Corner_2 = corner(temp_QR_binary_invert.*temp_QR_binary_invert_H2,'Harris','QualityLevel',0.2);
QR_Corner_3 = corner(temp_QR_binary_invert.*temp_QR_binary_invert_H3,'Harris','QualityLevel',0.2);
...
x_intersect_5 = mean([x_intersect_1,x_intersect_2,x_intersect_3]);
y_intersect_5 = mean([y_intersect_1,y_intersect_2,y_intersect_3]);

%if pdist([x_intersect_4,y_intersect_4;x_intersect_5,x_intersect_5])
...
temp_C_QR_binary_invert =bwareaopen(temp_C_QR_binary_invert, 30);
...
for i=1:size(mat_R_Area_index,2)
    ...
    if sum(in)-sum(on) == 4      
        ...
        in_aera = polyarea(in_points(1:4,1)',in_points(1:4,2)');
        area_ratio = edge_aera/in_aera;
        if round(area_ratio) >=4 && round(area_ratio)<=6
            find_num = find_num+1;
...
C_grid_2(C_grid_size_list(2)*3-1,C_grid_size_list(2)*3-1) = 1;
...
elseif version <7 && version >1
    ...
    code_length = r_m_size*r_m_size - 9*9 -8*9*2 - 2*version*4-25;
